<template>
    <div class=" h-full flex items-center justify-center p-3 main">
        <div class="flex flex-col  w-full sm:w-2/3 md:w-4/5 xl:w-[700px]">

            <div class="rounded-2xl drop-shadow-2xl bg-white   pt-0 ">
                <div class="bg-sky-500 rounded-t-2xl h-4"></div>
                <div class=" p-4">
                    <form class="w-full flex flex-col gap-6">
                        <div class="flex flex-col gap-2">
                            <div class="grid  gap-4 md:grid-cols-2 grid-cols-1">
                                <div class="w-full">
                                    <label class="text-sm" for="">Выберите выставку</label>
                                    <Select class="w-full" placeholder="Выберите выставку" v-model="exhibition"
                                        v-bind="exhibitionAttrs" :options="exhibitions" optionLabel="name"
                                        optionValue="id" />
                        ...
                        <div class="flex flex-col gap-2">
                            <div class="grid md:grid-cols-2 grid-cols-1 gap-4 items-start">
                                <div class="flex flex-col">
                                    <label class="text-sm" for="">Откуда вы узнали о выставке</label>
                                    <Select :options="sources" optionLabel="name" optionValue="name" v-model="fair"
                                        ...
                                        label="Откуда вы узнали о выставке" />
                                    <small v-if="errors.fair" class="text-red-500">{{ errors.fair }}</small>
                                </div>
                                <div>
                                    <label class="text-sm" for="">Специалист или частное лицо</label>
                                    <Select class="w-full" placeholder="Выберите" v-model="executor"
                                        v-bind="executorAttrs" :options="executorOptions" optionLabel="title"
                                        optionValue="name" />
                                    <small v-if="errors.executor" class="text-red-500">{{ errors.executor }}</small>
                                </div>

                            </div>
                            <div v-if="executor === 'company'"
                                class="grid md:grid-cols-2 grid-cols-1 items-center gap-4 justify-between">
                                ...
                                <div>
                                    <label class="text-sm" for="">Страна</label>
                                    <Select :options="countries" v-model="country" editable optionLabel="name"
                                        ...
                                        label="Страна/город" class="w-full" />
                                </div>
                            </div>
                        </div>
    ...
    <DownloadTicketDialog v-if="responseData" v-model="dialogStatus" :ticketUrl="responseData.qrValue"
...
<script lang="ts" setup>
    ...
    const { errors, handleSubmit, defineField, values, resetForm, setFieldValue } = useForm({
        initialValues: {
            name: '',
            email: '',
            phone: '',
